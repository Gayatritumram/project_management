...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query("SELECT t.status, COUNT(t) FROM Task t WHERE t.assignedByAdmin = :admin GROUP BY t.status")
    ...
    @Query("SELECT COUNT(t) FROM Task t WHERE t.assignedByAdmin = :admin AND t.startDate BETWEEN :start AND :end")
    Long countByDateRangeForAdmin(@Param("admin") ProjectAdmin admin,
                                  @Param("start") LocalDate start,
                                  @Param("end") LocalDate end);

    @Query("SELECT COUNT(t) FROM Task t WHERE t.assignedTo = :member AND t.startDate BETWEEN :start AND :end")
    Long countByDateRangeForMember(@Param("member") TeamMember member,
                                   @Param("start") LocalDate start,
                                   @Param("end") LocalDate end);
}
